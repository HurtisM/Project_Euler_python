...
# Using P059input.txt, a file containing the encrypted ASCII codes, and the knowledge that the plain text must
...
with open(FILE, 'r') as f:
    num_str = f.read().split(',')
    numbers = [int(num) for num in num_str]

encrypted_message = numbers# Example encrypted message (ASCII code numbers)

for key_tuple in product(range(97, 123), repeat=3):  # a-z
    key = list(key_tuple)
    decrypted_chars = []
    for i, char in enumerate(encrypted_message):
        decrypted_chars.append(char ^ key[i % 3])

    decrypted_text = ''.join(chr(c) for c in decrypted_chars)

    if ' the ' in decrypted_text:  # simple check for English text
        print("Key:", ''.join(chr(k) for k in key))
        print("Message:", decrypted_text)
        print("Sum of ASCII:", sum(decrypted_chars))
        break
